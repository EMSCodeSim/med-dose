...
export default function MedicationEngine({medication,close,record,openProtocol,onContextChange,initialPatient}:Props){
  ...
  const isDopamine=medication.id==="dopamine",
    ...
    adminConc=fieldConcentration?concentrationInUnit(fieldConcentration,concentrationUnit):0,conc=customConcentrationMode?Number(customConcentration):adminConc,eligibilityAge=ageRequired?effectiveAgeYears:path?.patient==="pediatric"?8:40,eligibility=path&&needsPatientInfo?genericEligibilityReason(path,eligibilityAge,kg):"",
