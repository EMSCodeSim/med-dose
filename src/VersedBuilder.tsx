...
export default function VersedBuilder({close,openProtocol,record,onContextChange,medicationOptions,selectMedication,initialPatient}:Props){
  ...
  const dosePreviewReady=medConfirmed&&!!concentration&&labelConfirmed&&!!indication&&!!route&&patientComplete;
  ...
  const submitConcentration=()=>{
    ...
    setStage(indication&&route&&patientComplete?"result":"indication");
  };
  const chooseConcentration=(value:number)=>{setConcentration(value);setLabelConfirmed(true);setSafetyConfirmed(false);setRecorded(false);setStage(indication&&route&&patientComplete?"result":"indication")};
  const chooseRoute=(value:string)=>{const fastEdit=dosePreviewReady;setRoute(value);setSafetyConfirmed(false);setRecorded(false);setStage(fastEdit?"result":isImminent(indication)?"safety":"patient")};
  const board:CalculationBox[]=[
    {id:"concentration",label:"CONCENTRATION",value:`${fmt(concentration||0)} mg/mL`,detail:labelConfirmed?"Physical label confirmed":"Confirm physical label",complete:!!concentration&&labelConfirmed,active:stage==="concentration",available:!recorded,onClick:()=>editBox("concentration"),quickChoices:recorded?[]:[5].map(value=>({label:`${value} mg/mL`,selected:concentration===value&&labelConfirmed,onClick:()=>chooseConcentration(value)})).concat([{label:"Custom",selected:concentration===0,onClick:()=>{setConcentration(0);setLabelConfirmed(false);setStage("concentration")}}])},
    ...
    {id:"route",label:"ROUTE",value:route,detail:route&&indication&&routeChoices(indication).length===1?"Auto-filled":"Select approved route",complete:!!route,active:stage==="route",available:!recorded,onClick:()=>editBox("route"),quickChoices:recorded?[]:routeChoices(indication).map(value=>({label:value,selected:route===value,onClick:()=>chooseRoute(value)}))},
  ...
  return <MedicationBuilderShell medication={{name:"Midazolam (Versed)",subtitle:"Benzodiazepine",protocolId:"9070",vialLabel:"Midazolam"}} boxes={board} close={close} reset={close} calculationComplete={calculationReady} leftTools={<div id="versed-left-tools" className="versed-left-tools" aria-label="Report treatment and protocol tools"/>}>
      ...
      {stage!=="result"&&stage!=="safety"&&<section className="builder-workspace versed-inline-workspace" aria-label={`Edit ${stage}`}><button className="versed-inline-close" onClick={()=>setStage("result")} aria-label="Close selection">×</button>
        ...
        {stage==="route"&&<><small className="eyebrow">ROUTE</small><h1>Select route</h1><p className="screen-help">{indication?"Only routes approved for the selected indication are shown.":"Choose now; the indication selection will verify that the route is approved."}</p><div className="builder-options route-options">{routeChoices(indication).map(x=><button key={x} className={route===x?"selected":""} onClick={()=>chooseRoute(x)}><b>{x}</b><span>Select</span></button>)}</div></>}
