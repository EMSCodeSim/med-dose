...
export default function VersedBuilder({close,openProtocol,record,onContextChange,medicationOptions,selectMedication}:Props){
  ...
  return <main className="versed-builder">
    ...
    <div className="versed-layout">
      ...
      {stage==="result"&&!hasStarted&&<section className="versed-empty-workspace">{medicationReference}<b>Select any box</b><span>Build the calculation in the order that is fastest for you.</span></section>}
      ...
      {stage!=="result"&&stage!=="safety"&&<section className="builder-workspace versed-inline-workspace" aria-label={`Edit ${stage}`}>{medicationReference}<button className="versed-inline-close" onClick={()=>setStage("result")} aria-label="Close selection">×</button>
        {stage==="medication"&&<><small className="eyebrow">MEDICATION</small><h1>{addingMedication?"Add medication to this patient":"Select a DMP medication"}</h1><p className="screen-help">{addingMedication?"Patient information and the existing administration record will carry forward.":"Tap the medication in your hand. Medications still under review remain visible but locked."}</p><div className="versed-medication-list">{medicationOptions.map(option=><button key={option.id} disabled={!option.released} className={option.id==="midazolam"&&medConfirmed?"selected":""} onClick={()=>{if(option.id==="midazolam"){setAddingMedication(false);if(!recorded){setMedConfirmed(true);setSafetyConfirmed(false)}setStage("result")}else selectMedication(option.id,addingMedication?patientSnapshot:undefined)}}><b>{option.name}</b><span>{option.brand}</span><i>{option.released?option.id==="midazolam"?"Select":"Open":"Under review"}</i></button>)}</div></>}
