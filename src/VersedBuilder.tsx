...
export default function VersedBuilder({close,openProtocol,record,onContextChange}:Props){
  ...
  const submitConcentration=()=>{
    ...
    setStage(safetyConfirmed?"result":"indication");
  };
  const board:CalculationBox[]=[
    ...
    {id:"concentration",label:"CONCENTRATION",value:`${fmt(concentration||0)} mg/mL`,detail:labelConfirmed?"Physical label confirmed":"Confirm physical label",complete:!!concentration&&labelConfirmed,active:stage==="concentration",available:medConfirmed&&!recorded,onClick:()=>editBox("concentration")},
    {id:"indication",label:"INDICATION",value:indication,detail:indication?"DMP pathway selected":"Select reason for use",complete:!!indication,active:stage==="indication",available:!!concentration&&labelConfirmed&&!recorded,onClick:()=>editBox("indication")},
    {id:"route",label:"ROUTE",value:route,detail:route&&routeChoices(indication).length===1?"Auto-filled":"Select approved route",complete:!!route,active:stage==="route",available:!!indication&&!recorded,onClick:()=>editBox("route")},
    {id:"patient",label:"PATIENT",value:patientLabel,detail:patientComplete?(needsWeight?"Weight changes dose":"Only dose-changing information shown"):"Enter dose-changing information",complete:patientComplete,active:stage==="patient",available:!!route&&!recorded,onClick:()=>editBox("patient")},
    {id:"safety",label:"SAFETY",value:"All checks confirmed",detail:safetyConfirmed?"One confirmation":"Review complete safety list",complete:safetyConfirmed,active:stage==="safety",available:patientComplete&&!recorded,onClick:()=>editBox("safety")},
  ];
  ...
  return <main className="versed-builder">
    ...
    <div className="versed-layout">
      ...
      <section className="versed-dashboard" aria-label="Live Versed dose and treatment information">
        <header><small>LIVE CALCULATION</small><h1>Midazolam (Versed)</h1><b>{route||"Route pending"}</b></header>
        <div className={safetyConfirmed?"dashboard-dose ready":"dashboard-dose pending"}><div><small>GIVE DOSE</small><strong>{dose>0?`${fmt(dose)} mg`:"—"}</strong></div><div><small>DRAW VOLUME</small><strong>{volume>0?`${fmt(volume)} mL`:"—"}</strong></div></div>
        {!safetyConfirmed&&<button className="dashboard-required" onClick={()=>patientComplete?setStage("safety"):setStage(board.find(item=>!item.complete&&item.available!==false)?.id as Stage||"medication")}><b>Calculation not ready</b><span>Complete the red check on the left to unlock the final answer.</span><i>Open required check ›</i></button>}
        ...
        <button className="initial-record-dose dashboard-record" disabled={!safetyConfirmed||recorded||!(dose>0)||!(volume>0)} onClick={()=>{const time=Date.now();record({drug:"Midazolam (Versed)",reason:indication,route,dose,unit:"mg",volume,time,concentration:`${fmt(concentration||0)} mg/mL`,patient:patientLabel});setRecorded(true);setRecordedAt(time);setNow(time)}}><span><small>{recorded?"ADMINISTRATION RECORDED":"RECORD ADMINISTRATION"}</small><b>{route||"Route pending"}</b></span><strong>{dose>0&&volume>0?`${fmt(dose)} mg • ${fmt(volume)} mL`:"Complete required checks"}</strong><em>{recorded?"Timer started • saved to encounter report":safetyConfirmed?"Tap to record and start timer":"Final safety confirmation required"}</em></button>
      </section>
    </div>
    {stage!=="result"&&<div className="versed-popup-backdrop" onClick={()=>setStage("result")}><section className="builder-workspace versed-popup" role="dialog" aria-modal="true" aria-label={`Edit ${stage}`} onClick={e=>e.stopPropagation()}><button className="versed-popup-close" onClick={()=>setStage("result")} aria-label="Close popup">×</button>
