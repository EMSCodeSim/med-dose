...
export default function DoseTracker({entries,unit,total,totalVolume,maxTotal,repeatsLeft,repeatMinutes,secondsLeft,nextDose,concentration,drug,reason,record}:{entries:Entry[];unit:string;total:number;totalVolume:number;maxTotal:number;repeatsLeft:number;repeatMinutes:number;secondsLeft:number;nextDose:number;concentration:number;drug:Drug;reason:string;record:(dose:number)=>void}) {
  ...
  return <section className={`dose-tracker ${started?"has-doses":"first-dose"}`} aria-live="polite">
    <div className="tracker-head"><span><small>ADMINISTRATION</small><h2>{started?"Dose tracker":"Record medication given"}</h2></span>{started&&<b>{repeatsLeft} repeat{repeatsLeft===1?"":"s"} available</b>}</div>
    {!started?<>
      <div className="ready-dose"><span><small>READY TO RECORD</small><strong>{fmt(nextDose)} {unit}</strong></span><b>{fmt(nextDose/concentration)} mL</b></div>
      <div className="limit-inline">Protocol cumulative limit: <b>{fmt(maxTotal)} {unit}</b></div>
      {editor}
      {recordButton}
    ...
    <p className="tracker-foot">Recorded totals support documentation but do not replace reassessment or the ePCR.</p>
  </section>
}
