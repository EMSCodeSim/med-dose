...
function TreatmentPanel({age,kg,drug,indication,route,dose,openProtocol,selectSuggested}:{age:number|null;kg:number|null;drug:SupportedDrug;indication:string;route?:string;dose?:string;openProtocol:(x:ProtocolTarget)=>void;selectSuggested:(drug:SupportedDrug)=>void}) {
  ...
  return <div className="treatment-list">
    ...
    {drug==="epinephrine"&&/allergic|anaphylaxis/.test(indication.toLowerCase())&&<div className="adjunct-medications"><small>DMP 4090 • ADDITIONAL MEDICATION OPTIONS</small><h3>Continue this patient encounter</h3><p>The next medication opens with blank patient fields so every dose is independently verified.</p><button onClick={()=>selectSuggested("diphenhydramine")}><b>Diphenhydramine</b><span>If time and patient stability permit</span><i>Start ›</i></button><button onClick={()=>selectSuggested("methylprednisolone")}><b>Methylprednisolone</b><span>If time and patient stability permit; delayed effect—do not delay transport</span><i>Start ›</i></button><button onClick={()=>selectSuggested("albuterol")}><b>Albuterol</b><span>Only when wheezing is present; IM Epinephrine is given first</span><i>Start ›</i></button></div>}
