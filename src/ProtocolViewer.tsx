...
export default function ProtocolViewer({ target, close }: { target: ProtocolTarget; close: () => void }) {
  ...
  useEffect(()=>{let active=true;const task=getDocument({url:ORIGINAL,disableRange:true,disableStream:true});task.promise.then(doc=>{if(active){setPdf(doc);setStatus("")}}).catch(()=>{if(active){setError("The current DMP could not load. Connect to the internet once and use the original PDF below; after a successful load it is retained by the app cache.");setStatus("")}});return()=>{active=false;task.destroy()}},[]);
