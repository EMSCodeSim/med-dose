...
const DEFAULT_VISIBLE_IDS=["midazolam","fentanyl","ketamine"];
...
export default function AdminMedicationManager({medications,reviews,setReviews,openLegacyReview,close}:Props){
  ...
  const beginEdit=()=>{if(!selected||!currentData)return;setEditorData(clone(currentData));setEditorCatalog({...selected,protocol:{...selected.protocol}});setProtocolRevision(record?.protocolRevision||"July 2026");setError("");setSavedMessage("");setEditing(true)};
  ...
  const validateClinical=(data:JsonObject)=>{
    ...
    if(Array.isArray(data.paths)){
      ...
      for(const raw of data.paths as DosePath[]){
        ...
        if(!raw.route?.trim())throw new Error(`Route is required for ${raw.label||raw.id}.`);
        const f=raw.formula||{};
        const numeric=[f.amount,f.min,f.max].filter(v=>v!==undefined);
        if(numeric.some(v=>!Number.isFinite(v)||Number(v)<0))throw new Error(`Dose values cannot be negative for ${raw.label||raw.id}.`);
        if(f.kind==="range"&&f.min!==undefined&&f.max!==undefined&&f.min>f.max)throw new Error(`Minimum dose cannot exceed maximum dose for ${raw.label||raw.id}.`);
        if(f.kind==="perKg"&&f.amount===undefined)throw new Error(`Per-kg amount is required for ${raw.label||raw.id}.`);
      }
    }
  };
  const saveDraft=(keepEditing=false)=>{
    ...
    try{
      const nextData={...editorData,id:selected.id,name:editorCatalog.name,protocolId:editorCatalog.protocol.id,page:editorCatalog.protocol.page};
  ...
  const createMedication=()=>{
    ...
    const next={...state,[id]:{...initialAdminRecord(id),clinicalRevision:0,reviewStartedAt:Date.now(),draft:clinical,draftCreatedAt:Date.now()}};persist(next);setReviews({...reviews,[id]:{}});setAdding(false);setNewMed({name:"",brand:"",category:"",protocolId:"",protocolName:"",page:""});setSelectedId(id);setEditorCatalog(item);setEditorData(clinical);setProtocolRevision("July 2026");setEditing(true);setError("");
  };
...
function ConcentrationEditor({values,onChange}:{values:Concentration[];onChange:(v:Concentration[])=>void}){
  const update=(index:number,key:string,value:any)=>onChange(values.map((item,i)=>i===index?{...item,[key]:value}:item));
  return <div className="admin-stack-editor">{values.map((item,index)=><article className="admin-concentration-card" key={index}><div className="admin-form-grid compact"><Field label="Display label"><input value={item.label||""} onChange={e=>update(index,"label",e.target.value)}/></Field><Field label="Medication amount"><input type="number" step="any" value={item.amount??""} onChange={e=>update(index,"amount",num(e.target.value))}/></Field><Field label="Amount unit"><input value={item.amountUnit||""} onChange={e=>update(index,"amountUnit",e.target.value)}/></Field><Field label="Vial volume (mL)"><input type="number" step="any" value={item.volume??""} onChange={e=>update(index,"volume",num(e.target.value))}/></Field><Field label="Calculated concentration"><input type="number" step="any" value={item.concentration??""} onChange={e=>update(index,"concentration",num(e.target.value))}/></Field><Field label="Concentration unit"><input value={item.concentrationUnit||""} onChange={e=>update(index,"concentrationUnit",e.target.value)}/></Field></div><button className="danger text-button" onClick={()=>onChange(values.filter((_,i)=>i!==index))}>Remove concentration</button></article>)}<button className="admin-add-row" onClick={()=>onChange([...values,{label:"",amount:undefined,amountUnit:"mg",volume:undefined,volumeUnit:"mL",concentration:undefined,concentrationUnit:"mg/mL"}])}>+ Add concentration</button></div>;
}
...
function ClinicalRecord({data}:{data:JsonObject}){
  ...
  return <section className="admin-clinical-record"><div className="admin-clinical-intro"><small>COMPLETE MEDICATION RECORD</small><b>Review every section against the current protocol.</b><span>Select “Edit medication fields” to make changes using the structured editor.</span></div>{entries.map(([key,value])=><article key={key}><h3>{key.replace(/([A-Z])/g," $1").replace(/^./,letter=>letter.toUpperCase())}</h3>{Array.isArray(value)?<div className="admin-value-list">{value.map((item,index)=><div key={index}>{typeof item==="object"&&item!==null?<pre>{JSON.stringify(item,null,2)}</pre>:<span>{String(item)}</span>}</div>)}</div>:typeof value==="object"&&value!==null?<pre>{JSON.stringify(value,null,2)}</pre>:<p>{String(value??"—")}</p>}</article>)}</section>;
}
