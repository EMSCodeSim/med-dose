...
const baseData=(med:CatalogMedication):JsonObject=>{
  ...
  if(specialDefaults[med.id])return enrichMedicationAdminData(med,clone(specialDefaults[med.id]));
  return enrichMedicationAdminData(med,{id:med.id,name:med.name,protocolId:med.protocol.id,page:med.protocol.page,contraindications:[],concentrations:[],paths:[]});
};
...
export default function AdminMedicationManager({medications,reviews,setReviews,openLegacyReview,close}:Props){
  ...
  const saveDraft=(keepEditing=false)=>{
    ...
    try{
      ...
      const before=(loadClinicalOverrides()[selected.id] as JsonObject|undefined)||baseData(selected);
      const changes=diffSummary(before,nextData);
      updateRecord(selected.id,r=>({...r,protocolRevision:protocolRevision.trim()||CURRENT_DMP_PROTOCOL_REVISION,reviewStartedAt:r.reviewStartedAt||Date.now(),draft:nextData,draftCreatedAt:Date.now()}));
      resetSignatures(selected.id);
      setSavedMessage(changes.length?`Draft saved. ${changes.length} clinical section${changes.length===1?"":"s"} changed.`:"Draft saved. No differences from the published record yet.");
      setError("");
  ...
  const createMedication=()=>{
    ...
    const clinical:JsonObject=enrichMedicationAdminData(item,{id,name,protocolId:item.protocol.id,page:item.protocol.page,contraindications:[],concentrations:[],paths:[]});
...
function StructuredMedicationEditor({data,setData,catalog,setCatalog,protocolRevision,setProtocolRevision,saveSection,error,cancel,save}:{data:JsonObject;setData:(v:JsonObject)=>void;catalog:CatalogMedication;setCatalog:(v:CatalogMedication)=>void;protocolRevision:string;setProtocolRevision:(v:string)=>void;saveSection:()=>void;error:string;cancel:()=>void;save:()=>void}){
  ...
  return <section className="admin-structured-editor">
    ...
    <EditorSection title="Concentrations" note="Department stock/vial concentrations. Common EMS stock is prefilled when confidence is high; verify the department inventory and physical label before approval." onSave={saveSection}><ConcentrationEditor values={Array.isArray(data.concentrations)?data.concentrations:[]} onChange={values=>set("concentrations",values)}/></EditorSection>
    {isGeneric?<>
      ...
      <EditorSection title="Administration" onSave={saveSection}><StringListEditor values={arrays(data.administration)} onChange={v=>set("administration",v)} addLabel="Add administration instruction"/></EditorSection>
      <EditorSection title="Special considerations / notes" onSave={saveSection}><StringListEditor values={arrays(data.notes)} onChange={v=>set("notes",v)} addLabel="Add note"/></EditorSection>
    </>}
    {error&&<div className="admin-med-error">{error}</div>}
    <div className="admin-editor-bottom"><button onClick={cancel}>Cancel</button><button className="primary" onClick={save}>Save medication draft</button></div>
  </section>;
}

function EditorSection({title,note,onSave,children}:{title:string;note?:string;onSave:()=>void;children:ReactNode}){return <section className="admin-editor-section"><header><div><h3>{title}</h3>{note&&<p>{note}</p>}</div><button onClick={onSave}>Save section</button></header>{children}</section>}
...
function StringListEditor({values,onChange,addLabel}:{values:string[];onChange:(v:string[])=>void;addLabel:string}){return <div className="admin-list-editor">{values.map((value,index)=><div className="admin-list-row" key={index}><input value={value} onChange={e=>onChange(values.map((item,i)=>i===index?e.target.value:item))}/><button className="danger" onClick={()=>onChange(values.filter((_,i)=>i!==index))}>Remove</button></div>)}<button className="admin-add-row" onClick={()=>onChange([...values,""])}>+ {addLabel}</button></div>}
function KeyValueEditor({values,onChange}:{values:JsonObject;onChange:(v:JsonObject)=>void}){const entries=Object.entries(values);return <div className="admin-stack-editor">{entries.map(([key,value])=><div className="admin-key-value" key={key}><input className="key" value={key} onChange={e=>{const next={...values};delete next[key];next[e.target.value]=value;onChange(next)}}/><input value={String(value??"")} onChange={e=>onChange({...values,[key]:e.target.value})}/><button className="danger" onClick={()=>{const next={...values};delete next[key];onChange(next)}}>Remove</button></div>)}<button className="admin-add-row" onClick={()=>{let key="newRule",n=2;while(key in values)key=`newRule${n++}`;onChange({...values,[key]:""})}}>+ Add dose rule</button></div>}
...
function PathsEditor({medicationName,values,onChange}:{medicationName:string;values:DosePath[];onChange:(v:DosePath[])=>void}){
  const update=(index:number,patch:Partial<DosePath>)=>onChange(values.map((item,i)=>i===index?{...item,...patch}:item));
  const add=()=>{const seed=`${slugify(medicationName)}-${Date.now().toString(36)}`;onChange([...values,{id:seed,label:"New indication",agent:medicationName,patient:"adult",route:"IV/IO",formula:{kind:"fixed",amount:1,unit:"mg"},repeat:"",administration:"",protocol:"",monitoring:[],special:[]}])};
  const duplicate=(index:number)=>{const source=clone(values[index]) as DosePath;source.id=`${source.id}-copy-${Date.now().toString(36)}`;source.label=`${source.label} — new route`;onChange([...values,source])};
  return <div className="admin-path-list">{values.map((path,index)=><article className="admin-path-card" key={path.id||index}><header><div><small>TREATMENT PATH {index+1}</small><b>{path.label||"Untitled path"}</b></div><div><button onClick={()=>duplicate(index)}>Duplicate for new route</button><button className="danger" onClick={()=>onChange(values.filter((_,i)=>i!==index))}>Remove path</button></div></header><div className="admin-form-grid"><Field label="Path ID"><input value={path.id||""} onChange={e=>update(index,{id:e.target.value})}/></Field><Field label="Indication / use"><input value={path.label||""} onChange={e=>update(index,{label:e.target.value})}/></Field><Field label="Medication / agent"><input value={path.agent||""} onChange={e=>update(index,{agent:e.target.value})}/></Field><Field label="Patient group"><select value={path.patient||"adult"} onChange={e=>update(index,{patient:e.target.value})}>{patientGroups.map(x=><option key={x}>{x}</option>)}</select></Field><Field label="Minimum age (years)"><input type="number" min="0" step="any" value={path.minAge??""} onChange={e=>update(index,{minAge:num(e.target.value)})}/></Field><Field label="Maximum age (years)"><input type="number" min="0" step="any" value={path.maxAge??""} onChange={e=>update(index,{maxAge:num(e.target.value)})}/></Field><Field label="Approved route"><input value={path.route||""} onChange={e=>update(index,{route:e.target.value})}/></Field><Field label="Protocol / indication reference"><input value={path.protocol||""} onChange={e=>update(index,{protocol:e.target.value})}/></Field></div><FormulaEditor formula={path.formula||{kind:"fixed",amount:1,unit:"mg"}} onChange={formula=>update(index,{formula})}/><div className="admin-form-grid"><Field label="Repeat instructions"><textarea value={path.repeat||""} onChange={e=>update(index,{repeat:e.target.value})}/></Field><Field label="Administration instructions"><textarea value={path.administration||""} onChange={e=>update(index,{administration:e.target.value})}/></Field><Field label="Repeat interval (minutes)"><input type="number" min="0" step="any" value={path.repeatAfterMinutes??""} onChange={e=>update(index,{repeatAfterMinutes:num(e.target.value)})}/></Field><Field label="Maximum administrations"><input type="number" min="1" step="1" value={path.maxAdministrations??""} onChange={e=>update(index,{maxAdministrations:num(e.target.value)})}/></Field><Field label="Maximum cumulative dose"><input type="number" min="0" step="any" value={path.maxCumulative??""} onChange={e=>update(index,{maxCumulative:num(e.target.value)})}/></Field><Field label="Maximum cumulative per kg"><input type="number" min="0" step="any" value={path.maxCumulativePerKg??""} onChange={e=>update(index,{maxCumulativePerKg:num(e.target.value)})}/></Field><Field label="Absolute cumulative maximum"><input type="number" min="0" step="any" value={path.absoluteCumulativeMax??""} onChange={e=>update(index,{absoluteCumulativeMax:num(e.target.value)})}/></Field><Field label="Suggested concentration"><input type="number" min="0" step="any" value={path.suggestedConcentration??""} onChange={e=>update(index,{suggestedConcentration:num(e.target.value)})}/></Field><Field label="Concentration unit"><input value={path.concentrationUnit||""} onChange={e=>update(index,{concentrationUnit:e.target.value})}/></Field><Field label="Base contact requirement"><textarea value={typeof path.baseContact==="string"?path.baseContact:""} onChange={e=>update(index,{baseContact:e.target.value})}/></Field></div><label className="admin-checkbox"><input type="checkbox" checked={!!path.requiresWeight} onChange={e=>update(index,{requiresWeight:e.target.checked})}/><span>Weight required for this pathway</span></label><h4>Monitoring</h4><StringListEditor values={arrays(path.monitoring)} onChange={monitoring=>update(index,{monitoring})} addLabel="Add monitoring item"/><h4>Special considerations</h4><StringListEditor values={arrays(path.special)} onChange={special=>update(index,{special})} addLabel="Add special consideration"/></article>)}<button className="admin-add-row" onClick={add}>+ Add indication / route / dose path</button></div>;
}
function FormulaEditor({formula,onChange}:{formula:DoseFormula;onChange:(v:DoseFormula)=>void}){const set=(key:string,value:any)=>onChange({...formula,[key]:value});return <section className="admin-formula"><h4>Dose formula</h4><div className="admin-form-grid compact"><Field label="Formula type"><select value={formula.kind||"fixed"} onChange={e=>onChange({kind:e.target.value,unit:formula.unit||"mg"})}>{formulaKinds.map(x=><option key={x}>{x}</option>)}</select></Field><Field label="Dose unit"><select value={formula.unit||"mg"} onChange={e=>set("unit",e.target.value)}>{doseUnits.map(x=><option key={x}>{x}</option>)}</select></Field>{formula.kind==="fixed"&&<Field label="Fixed dose"><input type="number" min="0" step="any" value={formula.amount??""} onChange={e=>set("amount",num(e.target.value))}/></Field>}{formula.kind==="perKg"&&<><Field label="Dose per kg"><input type="number" min="0" step="any" value={formula.amount??""} onChange={e=>set("amount",num(e.target.value))}/></Field><Field label="Minimum dose"><input type="number" min="0" step="any" value={formula.min??""} onChange={e=>set("min",num(e.target.value))}/></Field><Field label="Maximum dose"><input type="number" min="0" step="any" value={formula.max??""} onChange={e=>set("max",num(e.target.value))}/></Field></>}{formula.kind==="range"&&<><Field label="Minimum dose"><input type="number" min="0" step="any" value={formula.min??""} onChange={e=>set("min",num(e.target.value))}/></Field><Field label="Maximum dose"><input type="number" min="0" step="any" value={formula.max??""} onChange={e=>set("max",num(e.target.value))}/></Field></>}{formula.kind==="instruction"&&<Field label="Dose instruction"><textarea value={formula.text||""} onChange={e=>set("text",e.target.value)}/></Field>}</div>{formula.kind==="ageBands"&&<AgeBands bands={formula.bands||[]} onChange={bands=>set("bands",bands)}/>}</section>}
function AgeBands({bands,onChange}:{bands:Array<{min:number;max:number;amount:number;label:string}>;onChange:(v:Array<{min:number;max:number;amount:number;label:string}>)=>void}){return <div className="admin-age-bands">{bands.map((band,index)=><div className="admin-age-band" key={index}><input placeholder="Label" value={band.label||""} onChange={e=>onChange(bands.map((x,i)=>i===index?{...x,label:e.target.value}:x))}/><input type="number" placeholder="Min age" value={band.min??""} onChange={e=>onChange(bands.map((x,i)=>i===index?{...x,min:Number(e.target.value)}:x))}/><input type="number" placeholder="Max age" value={band.max??""} onChange={e=>onChange(bands.map((x,i)=>i===index?{...x,max:Number(e.target.value)}:x))}/><input type="number" placeholder="Dose" value={band.amount??""} onChange={e=>onChange(bands.map((x,i)=>i===index?{...x,amount:Number(e.target.value)}:x))}/><button className="danger" onClick={()=>onChange(bands.filter((_,i)=>i!==index))}>Remove</button></div>)}<button className="admin-add-row" onClick={()=>onChange([...bands,{min:0,max:1,amount:1,label:"New age band"}])}>+ Add age band</button></div>}
