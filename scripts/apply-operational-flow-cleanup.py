...
if old not in s: raise SystemExit('dashboard primary anchor not found')
...
if old not in s: raise SystemExit('dashboard repeat anchor not found')
s=s.replace(old,new,1)
p.write_text(s)
...
if old not in s: raise SystemExit('safety header anchor not found')
s=s.replace(old,new,1)
# Add repeat action and move infusion into dashboard
